...
class Intents:
    ...
    async def intent_weather_dnipro(self, **kwargs) -> dict | str:
        ...
        forecast = self._collapse_weather(forecast)
        return {
            "current": current,
            "forecast": forecast
        }
    ...
    async def intent_hass(self, room: str, device: str, **kwargs) -> dict | str:
        ...
        if not entity_id:
            entity_id = await self._get_entity_by_room_and_friendly_name(
                room, device)
...
"""
    ...
    def _collapse_weather(self, data):
        periods = {
            "night": range(0, 6),
            "morning": range(6, 12),
            "day": range(12, 18),
...
class Tools(Intents):
    ...
    def _build_schema(self) -> list:
        ...
        for attr_name in dir(self):
            ...
            if not hasattr(method, "_tool_meta"):
                continue
    ...
    def get_tools_schema(self, exclude: list = []) -> list:
        return [
            t for t in self._tools_schema
            if t["function"]["name"] not in exclude
        ]

    @tool(description="Web search")
    async def tool_search(self, query: str, **kwargs) -> list | None:
        return await self.intent_search(query=query) or []
